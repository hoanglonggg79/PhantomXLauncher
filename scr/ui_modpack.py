...
_MAX_CONCURRENT = 6
_USER_AGENT = "PhantomXLauncher/1.2.0 (hoanglonggg79@gmail.com)"
...
class ModpackInstallWorker(QThread):
    ...
    def _install_fabric(self, mc_ver: str, loader_ver: str, gdir: str):
        java = self.mgr.find_java_for_version(mc_ver) or self.mgr.find_java() or "java"
        self.mgr.install_fabric(mc_ver, loader_ver, gdir, java_path=java, cb_log=self._log)

    def _install_quilt(self, mc_ver: str, loader_ver: str, gdir: str):
        java = self.mgr.find_java_for_version(mc_ver) or self.mgr.find_java() or "java"
        self.mgr.install_quilt(mc_ver, loader_ver, gdir, java_path=java, cb_log=self._log)
