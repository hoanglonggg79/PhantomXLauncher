...
class MinecraftManager:
    ...
    def install_vanilla(
        ...
        try:
            ...
            try:
                if cb_log:
                    cb_log("📦 Retrying install with legacy callback…")
    ...
    def install_forge(
        ...
        try:
            ...
            if cb_log:
                cb_log(f"⚠️  Permission error — retrying in 2s…")
            time.sleep(2)
            try:
                mcll.forge.install_forge_version(version_str, game_dir, java=java_path)
                logger.info(f"Forge installed (retry): {version_str}")
                return True
            except Exception as e2:
                logger.error(f"install_forge retry failed: {e2}")
                if cb_log:
                    cb_log(f"❌ Forge error (retry): {e2}")
                return False
        except Exception as e:
    ...
    def install_quilt(
        ...
        try:
            if cb_log:
                cb_log(f"🪡 Installing Quilt {loader_version or 'latest'} for {mc_version}…")
            lv = loader_version.strip() or None
            ...
            logger.info(f"Quilt installed: {mc_version}")
            return True
        except Exception as e:
            logger.error(f"install_quilt failed: {e}")
            if cb_log:
                cb_log(f"❌ Quilt error: {e}")
            return False
    ...
    def install_neoforge(
        ...
        try:
            if cb_log:
                cb_log(f"⚙️  Installing NeoForge {neoforge_version} for {mc_version}…")
            ...
            try:
                ...
                _loader.install(mc_version, game_dir, loader_version=lv)
            logger.info(f"NeoForge installed: {full_neoforge}")
            return True
        except PermissionError as pe:
    ...
    def install_java_runtime(
        ...
        try:
            ...
            if cb_log:
                cb_log(f"☕ Downloading Java runtime: {runtime_name}...")
            ...
            def set_max(m):
                state["max"] = m
                if cb_progress:
                    cb_progress(state["current"], m, "Downloading...")

            def set_progress(c):
                state["current"] = c
                if cb_progress:
                    cb_progress(c, state["max"], "Downloading...")
            ...
            if java_path and cb_log:
                cb_log(f"✅ Installation Successful: {java_path}")
            return java_path

        except Exception as e:
            logger.error(f"install_java_runtime failed: {e}")
            if cb_log:
                cb_log(f"❌ Error when installing Java runtime: {e}")
            return None
...
class MicrosoftAuthWorker(QThread):
    ...
    def run(self):
        ...
        try:
            # ✅ SỬA DÒNG NÀY: Unpack tuple ra thẳng 2 biến (hoặc dùng tuple unpacking)
...
def _safe_remove(path: Path, retries: int = 3, delay: float = 1.5):
    """Remove a file with retries on PermissionError (e.g. antivirus lock)."""
