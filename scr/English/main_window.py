...
class MainWindow(QMainWindow):
    ...
    def _on_modpack_installed(self, instance_name: str):
        ...
        self.signals.log.emit(
            f"✅ Modpack instance '{instance_name}' registered in launcher.", "SUCCESS"
        )
        self.signals.status_msg.emit(f"✅ Modpack ready: {instance_name}")
    ...
    def _install_instance(self, inst: Instance):
        if self.install_worker and self.install_worker.isRunning():
            QMessageBox.warning(
                self, "Busy", "An install is already in progress."
            )
            return
    ...
    def _on_install_done(self, ok: bool, name: str):
        ...
        if ok:
            self.signals.log.emit(
                f"✅ '{name}' installed and ready!", "SUCCESS"
            )
    ...
    def _launch_instance(self, inst: Instance):
        ...
        self.signals.log.emit(
            f"🚀 Launching '{inst.name}' as {username}…", "INFO"
        )
        self.launch_btn.setEnabled(False)
    ...
    def _check_updates(self):
        ...
        def _ver_tuple(v: str):
            """Convert e.g. '1.2.1' -> (1, 2, 1) for comparison."""
            try:
        ...
        def task():
            try:
                ...
                if r.status_code == 200:
                    ...
                    if remote_cleaned and _ver_tuple(remote_cleaned) > _ver_tuple(local_version):
                        ...
                        def show_update():
                            reply = QMessageBox.question(
                                ...
                                (
                                    ...
                                    "Open GitHub releases to download the latest version?"
                                ),
