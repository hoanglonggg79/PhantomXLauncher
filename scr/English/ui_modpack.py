...
# CurseForge API key (same as modpackinstaller.py)
...
class ModpackInstallWorker(QThread):
    ...
    def run(self):
        try:
            self._log(f"=== INSTALLING MODPACK: {self.instance_name} ===")
            ...
            self._log(f"✅ Instance '{self.instance_name}' created successfully!")
            self.done.emit(True, self.instance_name)

        except Exception as e:
            self._log(f"❌ Modpack install error: {e}")
    ...
    def _extract_and_parse(self) -> dict:
        self._log("📂 Extracting modpack archive...")
    ...
    def _setup_loader(self, manifest: dict) -> tuple[str, str, str]:
        self._log("⚙️  Setting up game base & loader...")
    ...
    def _install_vanilla(self, mc_ver: str, gdir: str):
        self._log(f"📦 Installing Minecraft {mc_ver}...")
    ...
    async def _download_mods(self, manifest: dict):
        self._log("⬇️  Downloading mods...")
        self.mods_dir.mkdir(parents=True, exist_ok=True)
        files = manifest.get("files", [])
        if not files:
            self._log("  No files to download.")
            return
    ...
    def _download_mods_sync(self, manifest: dict):
        """Fallback sync download when aiohttp is not available."""
        import requests
        self._log("⬇️  Downloading mods (sync mode)...")
        files = manifest.get("files", [])
        self.mods_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _apply_overrides(self):
        self._log("🚚 Applying overrides / configs...")
...
class ModpackTab(QWidget):
    """UI tab for installing modpacks from .zip / .mrpack files."""

    # Emitted when a new instance is created — main window should refresh
    ...
    def _build_ui(self):
        ...
        # Header
        h = QHBoxLayout()
        lbl = QLabel("📦 Modpack Installer")
        lbl.setObjectName("header")
        h.addWidget(lbl)
        h.addStretch()
        note = QLabel("Supports CurseForge & Modrinth formats")
        note.setObjectName("subtitle")
        h.addWidget(note)
        layout.addLayout(h)

        # Setup group
        setup_grp = QGroupBox("New Instance from Modpack")
        ...
        file_row.addWidget(QLabel("Modpack File:"))
        self.file_edit = QLineEdit()
        self.file_edit.setPlaceholderText("Select a .zip or .mrpack file...")
        ...
        info_lbl = QLabel(
            "ℹ️  The loader (Forge, Fabric, Quilt, NeoForge) will be installed automatically "
            "from scratch for this new instance."
        )
    ...
    def _start_install(self):
        ...
        if not name:
            QMessageBox.warning(self, "Required", "Please enter an instance name.")
            return
        if not zip_path or not Path(zip_path).exists():
            QMessageBox.warning(self, "Required", "Please select a valid modpack file.")
            return
        ...
        if name in self.inst_tab.instances:
            reply = QMessageBox.question(
                ...
                f"An instance named '{name}' already exists.\n"
                "Overwrite it? (All files will be replaced)",
        ...
        self.log_text.clear()
        self._append_log(f"📦 Starting modpack installation: {name}")
    ...
    def _cancel(self):
        ...
        self._reset_ui()
        self._append_log("⏹ Installation cancelled.")
    ...
    def _on_done(self, success: bool, instance_name: str):
        ...
        if success:
            self._append_log(
                f"✅ Modpack installed successfully as instance: '{instance_name}'"
            )
            ...
            QMessageBox.information(
                ...
                f"Select it in the Instances tab to launch.",
            )
        else:
            self._append_log("❌ Modpack installation failed. Check the log above.")
