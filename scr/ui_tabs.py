...
class InstanceTab(QWidget):
    ...
    def _build_ui(self):
        ...
        h.addWidget(lbl)
        h.addStretch()
        add_btn = QPushButton("➕ Instance mới")
    ...
    def delete_selected(self):
        ...
        reply = QMessageBox.question(
            self,
            "Xóa Instance",
            f"Xóa '{inst.name}'?\nThao tác này sẽ xóa TẤT CẢ tệp trong:\n{inst.game_dir}",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                logger.error(f"Lỗi xóa instance: {e}")
...
class NewInstanceDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Instance mới")
    ...
    def _build_ui(self):
        ...
        form = QGroupBox("Cấu hình Instance")
        fl = QVBoxLayout(form)

        row = QHBoxLayout()
        row.addWidget(QLabel("Tên:"))
        ...
        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Phiên bản MC:"))
        ...
        self.snap_cb = QCheckBox("Hiện bản snapshots")
        ...
        row4 = QHBoxLayout()
        row4.addWidget(QLabel("Phiên bản Loader:"))
    ...
    def _validate(self):
        if not self.name_edit.text().strip():
            QMessageBox.warning(self, "Bắt buộc", "Tên instance là bắt buộc.")
            return
        if not self.ver_combo.currentText().strip():
            QMessageBox.warning(self, "Bắt buộc", "Vui lòng chọn một phiên bản Minecraft.")
            return
        self.accept()
...
class ModTab(QWidget):
    ...
    def _build_ui(self):
        ...
        add_mod_btn = QPushButton("➕ Thêm Mod (.jar)")
        ...
        btn_row = QHBoxLayout()
        toggle_btn = QPushButton("⏯ Bật/Tắt Mod")
        ...
        open_dir_btn = QPushButton("📂 Thư mục Mods")
    ...
    def add_mod(self):
        if not self.current_instance:
            QMessageBox.warning(self, "Không có Instance", "Vui lòng chọn một instance trước.")
            return
        files, _ = QFileDialog.getOpenFileNames(
            self, "Chọn Mod JARs", "", "Mod JARs (*.jar)"
        )
        if not files:
            return
...
class MarketplaceTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.detail_name = QLabel("Chọn một mod để xem chi tiết.")
        ...
        inst_row.addWidget(QLabel("Cài đặt vào:"))
        ...
        btn_row = QHBoxLayout()
        self.download_btn = QPushButton("⬇️  Cài đặt Mod")
        self.download_btn.setObjectName("market")
        self.download_btn.setEnabled(False)
        self.download_btn.clicked.connect(self._download_selected)
        btn_row.addWidget(self.download_btn)

        self.open_web_btn = QPushButton("🌐 Mở trên Modrinth (Browser)")
    ...
    def _on_search_results(self, results: list):
        ...
        if not results:
            self.result_list.addItem("Không tìm thấy kết quả.")
            return
    ...
    def _on_versions_fetched(self, versions: list):
        ...
        if not versions:
            self.ver_combo.addItem("Không có phiên bản tương thích")
            self.download_btn.setEnabled(False)
            return
    ...
    def _download_selected(self):
        if not self.inst_combo.currentText():
            QMessageBox.warning(
                self, "Không có Instance", "Vui lòng chọn một instance để cài đặt mod."
            )
            return
        ...
        if not files:
            QMessageBox.warning(self, "Không có File", "Không tìm thấy file có thể tải xuống.")
            return
        ...
        if dest.exists():
            reply = QMessageBox.question(
                self,
                "Đã tồn tại",
                f"{filename} đã tồn tại trong thư mục mods. Ghi đè?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply != QMessageBox.StandardButton.Yes:
                return
        ...
        self.dl_status.setText(f"Đang tải xuống {filename}…")
    ...
    def _on_download_done(self, ok: bool, filename: str):
        ...
        self.download_btn.setEnabled(True)
        if ok:
            self.dl_status.setText(f"✅ {filename} Đã cài đặt!")
            self.install_signal.emit(f"✅ Mod đã được cài đặt: {filename}")
        else:
            self.dl_status.setText(f"❌ Tải xuống thất bại: {filename}")
...
class LogTab(QWidget):
    def __init__(self, parent=None):
        ...
        clear_btn = QPushButton("🧹 Dọn dẹp")
        clear_btn.clicked.connect(self._clear)
        ctrl.addWidget(clear_btn)

        save_btn = QPushButton("💾 Lưu Log")
...
class SettingsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        row = QHBoxLayout()
        row.addWidget(QLabel("Tên người dùng:"))
        self.username_edit = QLineEdit()
        self.username_edit.setPlaceholderText("OfflinePlayer")
        row.addWidget(self.username_edit)
        offline_l.addLayout(row)
        note = QLabel("ℹ️  Chỉ ở chế độ ngoại tuyến. Chơi Online chỉ khả dụng trên các máy chủ chạy ở chế độ ngoại tuyến (cracked server).")
        ...
        self.java_label = QLabel("🔍 Đang kiểm tra Java...")
        java_l.addWidget(self.java_label)

        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Đường dẫn Java tùy chỉnh:"))
        self.java_path_edit = QLineEdit()
        self.java_path_edit.setPlaceholderText("Để trống để tự động phát hiện")
        ...
        yt_btn.setToolTip("Kênh Youtube của HoangLong (PhantomX Dev)")
        ...
        self.close_launcher_cb = QCheckBox("Ẩn launcher khi trò chơi khởi động")
        misc_l.addWidget(self.close_launcher_cb)
        layout.addWidget(misc_grp)

        row_save = QHBoxLayout()
        row_save.addStretch()
        save_btn = QPushButton("💾 Lưu Setting")
    ...
    def _browse_java(self):
        p, _ = QFileDialog.getOpenFileName(
            self, "Chọn tệp thực thi Java (Java.exe)", "", "java*(*)"
        )
        if p:
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            QMessageBox.information(
                self, "Đã lưu", "✅ Settings đã được lưu thành công!"
            )
            return True
        except Exception as e:
