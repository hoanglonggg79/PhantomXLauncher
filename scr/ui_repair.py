...
class RepairWorker(QThread):
    ...
    def _scan_version(
        ...
        try:
            ...
            self.log.emit(f"⚠️  Không thể lấy manifest: {e}")
            ver_info = None
        ...
        if ver_info and json_path.exists():
            try:
                ...
                if not jar_path.exists():
                    self.log.emit(f"⚠️  Thiếu: {mc_ver}.jar → hàng đợi tải xuống")
                    queue.append((jar_url, jar_path, expected_sha1))
                elif expected_sha1:
                    actual = self._sha1_file(jar_path)
                    if actual != expected_sha1:
                        self.log.emit(f"  ⚠️  Bị hỏng: {mc_ver}.jar → hàng đợi tải xuống lại")
                        jar_path.unlink(missing_ok=True)
                        queue.append((jar_url, jar_path, expected_sha1))
                    else:
                        self.log.emit(f"  ✅ OK: {mc_ver}.jar")
            except Exception as e:
                self.log.emit(f"  ⚠️  Không thể xác minh JAR: {e}")
        elif not jar_path.exists():
            self.log.emit(f"  ⚠️  Thiếu: {mc_ver}.jar (không thể kiểm tra mà không có manifest)")
    ...
    def _scan_libraries(
        ...
        self.log.emit(f"  Thư viện: {len(needs_dl)} cần sửa chữa trong số {count}")
        return count, queue
    ...
    def _scan_assets(
        ...
        if not index_id:
            self.log.emit("  ⚠️ Không tìm thấy mục lục tài nguyên trong dữ liệu phiên bản")
            return 0, []
        ...
        if not index_path.exists() and index_url:
            try:
                ...
                self.log.emit(f"  📥 Đã tải xuống mục lục tài nguyên: {index_id}")
            except Exception as e:
                self.log.emit(f"  ⚠️  Không thể tải xuống mục lục tài nguyên: {e}")
                return 0, []
        ...
        try:
            ...
            self.log.emit(f"  ⚠️  Không thể phân tích mục lục tài nguyên: {e}")
            return 0, []

        objects = index_data.get("objects", {})
        count = len(objects)
        self.log.emit(f"  Đang kiểm tra {count} tài nguyên ({'sâu' if self.deep_repair else 'nhanh'})...")
                    ...
                    async with sem:
                        async with session.get(url, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                            if resp.status == 200:
                                ...
                                self.progress.emit(
                                    done_count[0], total,
                                    f"Đang tải xuống các tệp bị thiếu ({done_count[0]}/{total})..."
                                )
                                return
                            else:
        ...
        try:
            ...
            for lib in libs:
                ...
                try:
                    ...
                    self.log.emit(f"  ⚠️  Không thể trích xuất thư viện gốc từ {jar_path.name}: {e}")

            self.log.emit(f"  ✅ Đã tạo lại các tệp gốc ({extracted} file JARs đã được giải nén)")
            return True
        except Exception as e:
            self.log.emit(f"  ⚠️  Lỗi dựng lại bản gốc: {e}")
            return False
...
class RepairTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.deep_cb = QCheckBox(
            "Sửa chữa sâu (Deep Repair) — xác thực mã băm SHA-1 của tất cả tài nguyên (chậm hơn nhưng kỹ hơn)"
        )
    ...
    def _start_repair(self):
        ...
        if not inst:
            QMessageBox.warning(self, "Không có Instance", "Vui lòng chọn một instance để sửa chữa.")
            return

        self.log_text.clear()
        self._append_log(f"🔧 Bắt đầu sửa chữa cho: {inst.name}")
        self._append_log(f"   Phiên bản: {inst.version_id} | Loader: {inst.loader}")
    ...
    def _stop_repair(self):
        if self._worker:
            self._worker.stop()
        self._reset_ui()
        self._append_log("⏹ Sửa chữa đã bị dừng bởi người dùng.")
    ...
    def _on_error(self, msg: str):
        self._reset_ui()
        self._append_log(f"❌ Lỗi nghiêm trọng: {msg}")
        QMessageBox.critical(self, "Lỗi sửa chữa", f"Sửa chữa thất bại:\n{msg}")
    ...
    def _show_summary(self, s: dict):
        rebuilt = "✔ Đã tạo lại Thư viện gốc" if s.get("rebuilt_natives") else "— Không tạo lại Thư viện gốc"
        msg = (
            f"Đã hoàn tất sửa chữa\n\n"
            f"Đã quét:\n"
            f"  ✔ {s['scanned_jar']} Client JAR\n"
            f"  ✔ {s['scanned_libs']} Thư viện\n"
            f"  ✔ {s['scanned_assets']} Tài nguyên\n\n"
            f"Đã khôi phục:\n"
            f"  ✔ {s['restored_jar']} Client JAR\n"
            f"  ✔ {s['restored_libs']} Thư viện\n"
            f"  ✔ {s['restored_assets']} Tài nguyên\n"
            f"  {rebuilt}\n\n"
            f"Thời gian thực hiện: {s['elapsed']} giây"
        )
        QMessageBox.information(self, "Tổng kết sửa chữa", msg)
        self._append_log(
            f"✅ Sửa chữa hoàn tất trong {s['elapsed']} giây — "
            f"đã khôi phục: {s['restored_jar']} jar, "
